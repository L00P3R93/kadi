...
class VerifiedUserSeeder extends Seeder
{
    ...
    private function processUserThroughKadiApi(array $userData): void
    {
        ...
        // Step 2: Insert into kadi database (bcrypt hash only — never plaintext)
        $this->insertIntoKadiDatabase($userData, $customerId);
    ...
    private function insertIntoKadiDatabase(array $userData, int $customerId): void
    {
        try {
            ...
            DB::connection('kadi')->table('accounts')->insert([
                ...
                'email' => $userData['email'],
                // Same bcrypt hash as the users table ('password') so the game
                // site can verify logins with password_verify().
