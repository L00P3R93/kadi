...
>
    ...
    <div class="relative mx-auto max-w-6xl px-6">
        ...
        <div class="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            <div>
                <h1 class="font-cinzel text-3xl font-bold text-[#f5f5f0]">Ad Categories</h1>
                <p class="mt-1 text-sm text-[#6b6b6b]">Category pricing multipliers drive rewarded-video billing across every campaign.</p>
            </div>
            ...
            <button type="button" wire:click="openCreateModal" class="btn-casino-primary flex items-center gap-2 rounded-full px-5 py-2.5 text-xs whitespace-nowrap">
                <span class="text-base leading-none">+</span> New Category
            </button>
        </div>
        ...
        <p class="mt-4 text-[11px] text-[#6b6b6b]">
            Turning a category off stops it from being selectable for <em>new</em> campaigns — existing campaigns in that category keep running.
        </p>
    </div>
        ...
        >
            <div class="glass-card relative w-full max-w-md rounded-2xl border border-[#f5c542]/30 p-6">
                ...
                <form wire:submit="save" class="space-y-4">
                    ...
                    {{-- Name --}}
                    <div>
                        <label class="mb-1.5 block text-[10px] font-semibold uppercase tracking-widest text-[#6b6b6b]">Name</label>
                        <input type="text" wire:model="name" placeholder="e.g. Finance"
                    ...
                    {{-- Key --}}
                    <div>
                        <label class="mb-1.5 block text-[10px] font-semibold uppercase tracking-widest text-[#6b6b6b]">Key</label>
                        <input type="text" wire:model="key" placeholder="e.g. finance"
                               class="w-full rounded-lg border border-[#f5c542]/15 bg-black/30 px-3 py-2.5 text-sm text-[#f5f5f0] placeholder:text-[#6b6b6b] focus:border-[#f5c542]/50 focus:outline-none focus:ring-1 focus:ring-[#f5c542]/30">
                        <p class="mt-1 text-[10px] text-[#6b6b6b]">Lowercase, no spaces — used internally, not shown to advertisers.</p>
                        @error('key') <p class="mt-1 text-[11px] text-red-400">{{ $message }}</p> @enderror
                    </div>

                    {{-- Description --}}
                    <div>
                        <label class="mb-1.5 block text-[10px] font-semibold uppercase tracking-widest text-[#6b6b6b]">Description</label>
                        <textarea wire:model="description" rows="2" placeholder="Optional — shown to advertisers when picking a category"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" step="0.01" min="0.1" max="10" wire:model="pricing_multiplier"
                        ...
                        <p class="mt-1 text-[10px] text-[#6b6b6b]">Final view cost = base duration cost × this multiplier.</p>
                        @error('pricing_multiplier') <p class="mt-1 text-[11px] text-red-400">{{ $message }}</p> @enderror
                    </div>

                    {{-- Toggles --}}
                    <div class="flex items-center justify-between rounded-lg border border-[#f5c542]/10 bg-black/20 px-4 py-3">
                        <div>
                            <div class="text-xs font-semibold text-[#f5f5f0]">Requires Approval</div>
                            <div class="text-[10px] text-[#6b6b6b]">Campaigns in this category need admin review before going live</div>
                        </div>
                        <label class="relative inline-flex flex-shrink-0 cursor-pointer items-center">
                            <input type="checkbox" wire:model="requires_approval" class="peer sr-only">
                    ...
                    <div class="flex items-center justify-between rounded-lg border border-[#f5c542]/10 bg-black/20 px-4 py-3">
                        <div>
                            ...
                            <div class="text-[10px] text-[#6b6b6b]">Available for advertisers to pick on new campaigns</div>
                        </div>
                        <label class="relative inline-flex flex-shrink-0 cursor-pointer items-center">
