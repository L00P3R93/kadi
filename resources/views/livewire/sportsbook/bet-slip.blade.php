<div class="bg-[#111] flex flex-col h-full">
    ...
    <div class="flex-shrink-0 px-4 py-4 border-t border-[#222]">
        ...
        <button
            wire:click="placeBet"
            class="mt-3 w-full bg-[#f5c542] text-black font-bold py-2.5 rounded hover:bg-[#ffde74] transition text-sm tracking-wide disabled:opacity-40 disabled:cursor-not-allowed"
            @disabled(empty($selections) || !is_numeric($stake) || $stake <= 0)
        >
            PLACE BET
        </button>
