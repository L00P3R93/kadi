    ...
    >
        ...
        >
            ...
            @if ($hasError)
                ...
                @elseif ($balance !== null)
                    {{-- session('currency.code', 'KES') --}} {{ number_format($balance) }}
                @else
        ...
        <button
            type="button"
            @click="
            console.log('[wallet-balance] refresh clicked, cooldown=', cooldown);
            console.log('[wallet-balance] $wire =', $wire, ' $wire.__instance =', $wire?.__instance);
            if (cooldown) return;
            cooldown = true;
            setTimeout(() => cooldown = false, 10000);
            $wire.refreshWallet();
            console.log('[wallet-balance] refreshWallet() call dispatched');
        "
            :disabled="cooldown"
