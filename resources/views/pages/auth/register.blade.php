<x-layouts::auth :title="__('Register')">
    <div class="flex flex-col gap-6">
        ...
        <x-auth-header :title="__('Join KADI')" :description="__('Create your account to start playing.')" />
        ...
        <a href="{{ route('auth.google') }}"
           class="flex w-full items-center justify-center gap-3 rounded-lg bg-[#f5c542] px-4 py-3 text-sm font-semibold text-black transition hover:bg-[#e0b53d] shadow-lg shadow-[#f5c542]/20">
        ...
        <div class="flex items-center gap-3">
            ...
            <span class="text-xs text-zinc-500">{{ __('or sign up with email') }}</span>
            <div class="h-px flex-1 bg-zinc-700"></div>
        </div>
        ...
        <form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-6">
            @csrf
            <!-- Name -->
            <x-player-name-field :value="old('name')" :placeholder="__('Player name')" autofocus />
